  ...
  protected void drawActivity(DefaultProcessDiagramCanvas processDiagramCanvas, BpmnModel bpmnModel, 
    ...
    for (SequenceFlow sequenceFlow : flowNode.getOutgoingFlows()) {
      ...
      if (graphicInfoList != null && graphicInfoList.size() > 0) {
        ...
        GraphicInfo labelGra=new GraphicInfo();
        labelGra.setX((graphicInfoList.get(0).getX()+graphicInfoList.get(graphicInfoList.size()-1).getX())/2);
        labelGra.setY((graphicInfoList.get(0).getY()+graphicInfoList.get(graphicInfoList.size()-1).getY())/2);

        // Draw sequenceflow label
//        GraphicInfo labelGraphicInfo = bpmnModel.getLocationMap().get(sequenceFlow.getId());
//        if (labelGraphicInfo != null) {
          processDiagramCanvas.drawLabel(sequenceFlow.getName(), labelGra, true);
//        }
      }
    }
