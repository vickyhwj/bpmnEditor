...
/**
 * Represents a canvas on whi1ch BPMN 2.0 constructs can be drawn.
 * 
...
public class DefaultProcessDiagramCanvas {
  ...
  // Fonts
  protected static Font LABEL_FONT = new Font("宋体", Font.PLAIN, 16);
...
////    	if(previousPosition>text.length()) break;
////    	layout= measurer.nextLayout(boxWidth);
////    	lastLine = text.substring(previousPosition, measurer.getPosition());
//    	layouts.add(new TextLayout("大苏打速度", g.getFont(), g.getFontRenderContext()));
//    	
//    }
    ...
    while (measurer.getPosition() < characterIterator.getEndIndex() && currentHeight <= boxHeight) {
      ...
      if(currentHeight + height > boxHeight+1000) {
